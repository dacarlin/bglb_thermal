...
In order to test the hypothesis that individual residue identities in the BglB sequence are a trade off between function and thermal stability, we designed 125  point mutants covering 60 sequence positions of BglB and individually characterized the soluble protein expression, thermal stability, kcat, KM, and kcat/KM of each mutant. Statistical analyses reveal little correlation between Tm and kcat (PCC -0.04), KM (-0.14), and kcat/KM (-0.02) in the BglB system (however it should be noted that all correlations have a negative algebraic sign). The data set reported here allowed us perform a general analysis of the correlation between thermal stability and kinetic constants for 125 individual single steps in sequence-functional space immediately adjacent to the native BglB sequence (Hamming distance of 1), providing a system-level look at the structure-stability-functional relationships between residues in the BglB protein, and evidence that the idea that proteins must trade stability for function is not a general rule, and is in fact system-specific.
...
## Relationship between Tm and kcat, KM, and kcat/KM in BglB shows no general rules relating function to stability

In the BglB system, Tm and each of kcat, KM, and kcat/KM were not found to be correlated. Statistical analyses reveal that correlation between Tm and kcat (PCC -0.04), KM (-0.14), and kcat/KM (-0.02) in the BglB system. The data set reported here allowed us perform a general analysis of the correlation between thermal stability and kinetic constants for 125 individual single steps in sequence-functional space immediately adjacent to the native BglB sequence (Hamming distance of 1), providing a system-level look at the structure-stability-functional relationships between residues in the BglB protein, and evidence that the idea that proteins must trade stability for function is not a general rule, and is in fact system-specific.
...
## Putative function-stability tradeoffs in BglB
...
If the hypothesis was correct, you would expect that mutation of one of a pair of opposed-charged catalytic residues in close proximity would lead to great stability increases as the cost of function. However, in previous work in T4 Lysozyme the mutation E11H, where the negativel-ycharged catalyrirc reisue dus replaced with a residue of opposite charge, results in a negligdable increase in stabiloty, where the mutations E11A, E11F, and E11M confer stability increases of 2.6, 4.1, and 4.4 kcat/mol, respectively. And the mutation E11N is destabilizing by Tm -0.6. In the BglB system, the mutation E164R, which might be expected to stabilize the active site by trading a negative-egative interaction for a negative-positive, was found to not express, indicating that the mutation is destabuilizing.
